...
func setupTestStorage(t *testing.T) *PostgresStorage {
	...
	storage, err := NewPostgresStorage(ctx, pool)
	require.NoError(t, err)
...
func TestList_OrderedByUpdatedAt(t *testing.T) {
	...
	// Create memories in order with small delay to ensure different timestamps
...
func TestStorage_ImplementsInterface(t *testing.T) {
	// Compile-time check that PostgresStorage implements Storage interface
	var _ memory.Storage = (*PostgresStorage)(nil)
}
