// Package mo provides the Maybe monad for optional value handling.
// This implementation is compatible with mcp-genie's mo package.
package mo
...
// Map applies the given function to the contained value if present.
func Map[T, U any](m Maybe[T], fn func(T) U) Maybe[U] {
	if !m.valid {
		return Nothing[U]()
	}
...
// FlatMap applies the given function to the contained value if present.
// The function itself returns a Maybe.
func FlatMap[T, U any](m Maybe[T], fn func(T) Maybe[U]) Maybe[U] {
	if !m.valid {
		return Nothing[U]()
	}
	return fn(m.value)
